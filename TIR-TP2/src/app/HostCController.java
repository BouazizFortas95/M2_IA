...
public class HostCController implements Initializable {
	...
	@FXML
	private Button btn_client_connect, btn_client_gs;

	@FXML
	private ComboBox<String> cb_client_services;

	@FXML
	private HBox hb_client_logs, hb_client_services;

	@FXML
	private ListView<String> lv_client_logs;

	@FXML
	private Spinner<Integer> sp_client_port;
	...
	private boolean clientIsConnect = false;
				...
				if (clientIsConnect) {
					if (!serviceName.equals("stop")) {
						...
						try {
							...
							dos.writeUTF(serviceName.toString()+"-c");
							dos.flush();

							int serviceValue = dis.readInt();
							if (serviceName.equals("service-temp"))
								setMsg2BoitClient("Server : " + serviceValue + " °C");
