...
public class HostAController implements Initializable {
	...
	private final int serverPort = 1234, clientPort = 1235;
	...
	private HandelClient handelClient;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		services_list.add("service-temp");
					...
					if (!serviceName.equals("stop")) {
						...
						try {
							...
							if (serviceName.equals("service-temp")) {
								int saveTempo = dis.readInt();
								setMsg2BoitClient("Server : " + saveTempo + " °C");
							}
							if (serviceName.equals("stop")) {
								clientIsConnect = false;
								settClientStatus("-fx-background-color: light;", false, false, "Connect");
								setMsg2BoitClient(dis.readUTF());
								setClientServicesStatus(!clientIsConnect);
							}
	...
	private void runServer(ActionEvent event) {
		Runnable task = new Runnable() {
			...
			public void run() {
				if (serverIsRun) {
					setMsg2BoitServer("Opps!, Server is Running and you can't stop it.");
				} else {
					try {
						...
						if (server.isBound()) {
							...
							setMsg2BoitServer("Server is Running.");
						}
						do {
							handelClient = new HandelClient(server);
							handelClient.start();
						} while (serverIsRun);
